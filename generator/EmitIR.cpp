...
void EmitIR::FunctionDeclToIR(FunctionDecl *ptr){ 
    // 优化
    if (ptr->is_promotable && ptr->name != "main")
    {
        return;
    }
    ...
    if (ptr->block != nullptr)
    {
        ...
        {
            // 优化
            if (parm->is_promotable)
            {
                alloca_insts.emplace_back(nullptr);
                continue;
            }
        ...
        for (auto& arg: function->args())
        {
            // 优化
            // 不用到的参数不需要store
            if (alloca_insts[arg_idx] == nullptr)
            {
                ++arg_idx;
                continue;
            }
...
llvm::Value* EmitIR::BinaryExprToIR(BinaryExpr *ptr){
    ...
    else
    {
        ...
        {
            // 优化
            if (llvm::ConstantInt* const_lhs = llvm::dyn_cast<llvm::ConstantInt>(lhs))
            {
                // lhs为常量int
                // Algebraic Identity 恒等式，只对整型有用
                if (const_lhs->getValue() == 0 && ptr->opcode == "+")
                {
                    // 0+x=x
                    return rhs;
                }
                if (const_lhs->getValue() == 1 && ptr->opcode == "*")
                {
                    // 1*x=x
                    return rhs;
                }
                // Strength Reduction 强度削减
                if (const_lhs->getValue().isPowerOf2() && ptr->opcode == "*")
                {
                    // 2^n * x = x << n
                    auto value = const_lhs->getValue().getLimitedValue();
                    int n = 0;
                    while (value >= 2)
                    {
                        value >>= 1;
                    ...
                    return ir_builder.CreateShl(rhs, n);
                }
            }
            else if (llvm::ConstantInt* const_rhs = llvm::dyn_cast<llvm::ConstantInt>(rhs))
            {
                // rhs为常量int
                if (const_rhs->getValue() == 0 && ptr->opcode == "+")
                {
                    // x+0=x
                    return lhs;
                }
                if (const_rhs->getValue() == 1 && (ptr->opcode == "*" || ptr->opcode == "/"))
                {
                    // x*1=x, x/1=x
                    return lhs;
                }          
                // Strength Reduction 强度削减
                if (const_rhs->getValue().isPowerOf2() && ptr->opcode == "*")
                {
                    // x * 2^n = x << n
                    auto value = const_rhs->getValue().getLimitedValue();
                    int n = 0;
                    while (value >= 2)
                    {
                        value >>= 1;
                    ...
                    return ir_builder.CreateShl(lhs, n);
                }
                // if (const_rhs->getValue().isPowerOf2() && ptr->opcode == "/")
                // {
                //     // todo
                ...
                //         n++;
                //     }
                //     return ir_builder.CreateAShr(lhs, n);
                // }
            }
