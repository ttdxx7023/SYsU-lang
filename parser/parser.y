...
// TO-DO：你需要在这里补充更多的TOKEN
  ...
  if (t == "string_literal") {
    ...
    std::string type = std::string("char ")+std::string("[")+std::to_string(size)+std::string("]");
    yylval = new Tree("StringLiteral", "", s.str(), type);
...
| STRING T_STRING_LITERAL {
  int ls = atoi($1->type.substr($1->type.find('[')+1, $1->type.find(']')-$1->type.find('[')-1).c_str());
  int rs = atoi($2->type.substr($2->type.find('[')+1, $2->type.find(']')-$2->type.find('[')-1).c_str());
  int size = ls + rs;
  std::string type = std::string("char ") + std::string("[") + std::to_string(size) + std::string("]");
  std::string str = $1->value.substr(0, $1->value.size()-1) + $2->value.substr(1, $2->value.size());
  auto ptr = new Tree("StringLiteral", "", str, type);
  $$ = ptr; // 一定记得写这句话！
}
;
...
| T_IDENTIFIER T_L_PAREN FuncRParams T_R_PAREN {
  if($1->name == std::string("wk_puts")){
    // 处理字符串函数long long wk_puts(const char s[])
    std::string ret_type = "long long";
    // 只有一个参数
    ...
    ptr->addSon(func);
    auto param_imp = new Tree("ImplicitCastExpr", "NoOp", "", "");
    auto param_imp_son = new Tree("ImplicitCastExpr", "ArrayToPointerDecay", "", "");
