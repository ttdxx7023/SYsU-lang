...
int main(int argc, char **argv){
    ...
    // add function pass to module
    // error: This analysis pass was not registered prior to being queried
    ...
    // nopass func
    llvm::SmallSetVector<llvm::StringRef, 8> nopass;
    nopass.insert("my_getint");
    nopass.insert("median");
    nopass.insert("Vectordot");
    // nopass.insert("mult1");
    // nopass.insert("mult2");
    // nopass.insert("mult_combin");
    // nopass.insert("my_sqrt");

    // add function pass to every function
    for (auto &F: *M)
    {
        if (nopass.count(F.getName()))
        {
          break;
        }
